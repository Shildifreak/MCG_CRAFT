...
class Blockworld(object):
    ...
    def _init(self):
        ...
        # and some more incredible bad code
        import imp
        with open('Grassland.py', 'rb') as fp:
            main = imp.load_module(
            'main', fp, 'Grassland.py',
            ('.py', 'rb', imp.PY_SOURCE)
            )
        #import main
        #reload(main)
        ...
        self.TEXTURES = window.inventory
    ...
    def set_block(self,(x,y,z),BlockID=None):
        ...
        if BlockID != None:
            texture = self.TEXTURES[BlockID%len(self.TEXTURES)]
