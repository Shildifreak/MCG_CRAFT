    ...
    def add(self,message):
        ...
        if self.chunksize:
            ...
            if message[0] in ("setarea","delarea"):
                ...
                self.unserialized = collections.OrderedDict((k,v) for k,v in self.unserialized.items() if not self.in_chunk(message,chunkposition)) #M# was iteritems in python2
...
class Game(object):
    ...
    def launch_client(self):
        import subprocess
        command = ["python3",os.path.join(PATH,"client.py"),"--host=localhost","--port=%i" %self.socket_server.get_entry_port()]
